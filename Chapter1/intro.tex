...
\subsection{A Large Scale Mocap Database for Social Interactions (Chapter~\ref{chapter:dataset})  }
We build a large scale dataset capturing and measuring hundreds of participants in a social situation. We carefully designed a triadic social interaction, simulating a negotiation scenario, where voluntary social behaviors can naturally emerge. The objective of our dataset is to study the correlation among subtle socials signals transmitted during a social situation. The specific negotiation social scenario makes human behaviors more coherent, by putting all the subjects are in the same social situation. The natural social signals of the subjects in all sequences are measured by our markerless motion capture method in the CMU Panoptic Studio. Importantly, the multi-modal property of our dataset enables us to study the correlation among various input and output signals. 

Our dataset includes diverse other human motion data, which plays an important role for many computer vision and machine learning tasks by providing 3D and 2D motion annotations for human motions. Our dataset enables a popular hand keypoint detector~\cite{simon2017hand} in the OpenPose Library~\cite{openpose} and monocular 3D human motion capture~\cite{Xiang2019}. We publicly released out dataset. 
...
This thesis takes an important step to computationally measure and model social signals to ultimately endow machines with the nonverbal communication abilities. The work in this thesis has gained a great attentions from research community. In summary:
