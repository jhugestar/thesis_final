...
\begin{figure}[t]
	...
	\end{subfigure}\\
	\caption{Thesis contribution. (a) The Panoptic Studio with 521 unique views by 480 VGAs, 31 HDs, and 10 RGB+D sensors; (b) An example scene captured in the Panoptic Studio and measured 3D anatomical landmarks from bodies, faces, and hands; (c) A total motion capture result showing the reconstructed 3D mesh models; (d, left and middle) An example Haggling Game scene with measured social signals; (d, right)  an example output of social signal modeling (speaking prediction for the individual colored as red)\label{fig:teaser}}	
\end{figure}
%
%\begin{figure}[t]
%	\centering
