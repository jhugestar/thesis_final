...
%Social signal prediction tasks aim at mimicking human behaviors with two main requirements: (1) The predicted output signals should be within feasible human motion space showing realistic body behaviors, and (2) The predicted output should follow the social rules by responding the behaviors of input signals from communication partners. The first requirement is a necessary condition for the second requirement. The work in predicting single person's body motion focuses on the first requirement~\cite{mnih2012conditional, Fragkiadaki_2015_ICCV, jain2016structural}, and our social signal prediction introduces the additional requirement of (2).
%
%However, evaluating the prediction output is not straightforward, due to the inherent diversity of human behaviors. Given an input signals of others, there could be several possible reaction of humans, which all can be possible solution, if it is within our social norms. A common evaluation metric, L2 distance, may penalize the realistic motion if it is far from the ground truth motion measured in our system. This is one of the biggest challenging in evaluating the performance of the social signal prediction, and it also related to the loss function in training our models. 
%
%Human study by assessing the output of the prediction and evaluate the quality can be another way. However, we found that the evaluation is also subject to the way of visualizing the skeletons. For example, we found that showing skeletons provides different perception about the motion, compared to showing the original raw videos, because, for example, the skeleton visualization may hide the details of the subjects. Furthermore, human study requires a lot of effort and hard to be frequently used as a way to evaluate models. 
%
%There are some signals where L2 distance works well. Examples include binary classification tasks such as speaking classification, turn taking. It is also applicable in social formation in general. For the body and face motion, however, we found that it mislead the evaluation, showing that a mean pose showing a strong performance in this evaluation metric.
